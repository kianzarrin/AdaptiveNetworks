...
public struct OutlineData {
    ...
    public OutlineData(Bezier3 bezier, float width, bool smoothA, bool smoothD, float angleA, float angleD, float wireHeight) {
        ...
        float hw = 0.5f * width;
        SmoothA = smoothA;
        DirA = bezier.DirA().normalized;
        SmoothD = smoothD;
        DirD = bezier.DirD().normalized;
    ...
    public OutlineData(Bezier3 bezierA, Bezier3 bezierD, float width, float angleA, float angleD, float wireHeight) {
        float hw = width * .5f;
        DirA = -bezierA.DirA().normalized;
        DirD = -bezierD.DirA().normalized;
        SmoothA = SmoothD = true; //transitions are always smooth.
